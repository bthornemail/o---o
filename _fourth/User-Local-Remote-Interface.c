 ...
 *
 * This gives Tetragrammatron a stable adjudication invariant independent of
 * which sector a scope maps to, and it lines up with the standing Polybius
 * diagonal witnesses, which resolve the same 0x1E under both XOR and SUM.
 * ============================================================================
 */
 ...
 *      clock state, not synchronization authority. They remain available --
...
static OmiClockBand omi_classify_clock_state(uint16_t x) {
    ...
    for (int i = 0; i < 16; i++) {
        ...
        uint32_t bit_j = (uint32_t)((x >> ((i + 1) % 16)) & 1u);
...
#define OMI_CENTROID_PRIMARY_STATE   0x0000u /* address 0x0000, azimuth 0 degrees   */
#define OMI_CENTROID_PRIMARY_AZIMUTH 0x00u
#define OMI_CENTROID_ANTIPODAL_AZIMUTH 0x80u /* 180 degrees */

/* the antipodal centroid state is the r0-reflection of the primary centroid --
 * the same alternating-bit involution the clock already uses as its base
 * reflection generator, applied once at the reference level (0x0000 -> 0xAAAA) */
static inline uint16_t omi_centroid_state(uint8_t omnion) {
    return (omnion & 0x01u) ? omi_r0_reflection(OMI_CENTROID_PRIMARY_STATE)
                             : OMI_CENTROID_PRIMARY_STATE;
}
 ...
 * -- Section 11.4 below is the sync authority. */
static OmiGeometricVector omi_resolve_vector_to_centroid(uint16_t cons_address, uint8_t current_omnion) {
    ...
    uint8_t row = (uint8_t)((cons_address >> 8) & 0xFFu);
    uint8_t col = (uint8_t)(cons_address & 0xFFu);
...
/* ---- 11.4 Centroid resolution: the sync authority ------------------------- */
...
/* ---- 11.5 OMNION -> physical/logical frequency convergence ---------------- */

#define OMI_ATOMIC_CLOCK_FREQUENCY_HZ 9192631770ULL /* Cs-133 hyperfine transition, SI second */

typedef struct {
    uint16_t state_word;
    uint64_t step_accumulator;
    uint8_t  current_azimuth;   /* 0x00 or 0x80 -- reuses Section 10's mapping */
    uint64_t current_frequency; /* 0, or the SI atomic-clock constant          */
} OmiReconciledOmnionClock;

static OmiReconciledOmnionClock omi_process_centroid_alignment(uint16_t current_state, uint8_t omnion) {
    OmiReconciledOmnionClock clock;
    clock.state_word = current_state;
    clock.step_accumulator = 0;
    clock.current_azimuth = omi_omnion_azimuth_byte(omnion); /* Section 10 */
    clock.current_frequency = (omnion & 0x01u) ? OMI_ATOMIC_CLOCK_FREQUENCY_HZ : 0ULL;
    return clock;
}
...
/* AUTHORITY comparison: synchronized iff local_sync == remote_sync, where
...
int main(void) {
    ...
    printf("  Centroid state: primary(OMNION=0)=0x%04X  antipodal(OMNION=1)=0x%04X\n",
    ...
    OmiReconciledOmnionClock rc0 = omi_process_centroid_alignment(0x0000u, 0u);
    OmiReconciledOmnionClock rc1 = omi_process_centroid_alignment(0x0000u, 1u);
    printf("  OMNION=0 -> azimuth=0x%02X frequency=%" PRIu64 " Hz\n",
           rc0.current_azimuth, (uint64_t)rc0.current_frequency);
    printf("  OMNION=1 -> azimuth=0x%02X frequency=%" PRIu64 " Hz (Cs-133 SI second)\n\n",
           rc1.current_azimuth, (uint64_t)rc1.current_frequency);
    ...
    printf("      [AUTHORITY centroid resolution, OMNION=0] %s (local=0x%04X remote=0x%04X)\n",
    ...
    printf("      [AUTHORITY centroid resolution, OMNION=0] %s (local=0x%04X drifted=0x%04X)\n\n",
