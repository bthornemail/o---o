 ...
 * Hamming [7,4,3] codeword. OMINO is the extra overall-parity bit that lifts
 ...
 *
 * This section closes the loop between the two: OMINO is a single bit, and
 * a single bit is exactly enough information to distinguish two antipodal
 * points on the azimuth circle. omi_OMINO_azimuth_byte maps OMINO=0 to the
 * reference orientation 0x00 (0 degrees) and OMINO=1 to the antipodal
...
/* ---- 10.1 LOGOS/NOMOS/PATHOS/OMINO (# 4, # Miquel Extended Encoding Rule) --- */
...
static OmiEpistemicCheck omi_compute_epistemic_check(OmiScopeQuartet q) {
    ...
    c.OMINO = (uint8_t)(c.logos ^ c.nomos ^ q.fs ^ c.pathos ^ q.gs ^ q.rs ^ q.us);
    return c;
}

/* Hamming position order: 1=LOGOS 2=NOMOS 3=FS 4=PATHOS 5=GS 6=RS 7=US.
 * Index 0 is unused; extension parity (OMINO) is carried separately. */
typedef struct {
    uint8_t bit[8];
} OmiHammingCodeword;
...
typedef enum {
    ...
    OMI_HAMMING_CORRECTED_EXTENSION, /* the OMINO bit itself was the flipped bit */
    OMI_HAMMING_DOUBLE_ERROR_DETECTED
} OmiHammingStatus;

/* extended [8,4,4] SECDED check: syndrome plus the received OMINO bit
 * together distinguish "no error", "one data/check bit flipped",
 * "OMINO itself flipped", and "uncorrectable double-bit error".        */
static OmiHammingStatus omi_hamming_secded_check(OmiHammingCodeword *cw, uint8_t received_OMINO) {
    uint8_t syndrome = omi_hamming_syndrome(cw);

    uint8_t recomputed_OMINO = 0;
    for (int i = 1; i <= 7; i++) recomputed_OMINO ^= cw->bit[i];

    uint8_t total_parity = (uint8_t)(recomputed_OMINO ^ received_OMINO);
    ...
    if (syndrome == 0 && total_parity == 1) {
        return OMI_HAMMING_CORRECTED_EXTENSION; /* flip received_OMINO at the caller */
    }
...
/* ---- 10.3 The 0-degree OMINO model --------------------------------------- */

/* OMINO is one bit -- exactly enough to select between two antipodal
 * azimuth positions. OMINO=0 is the reference orientation (0 degrees).
 * OMINO=1 is the antipodal orientation (180 degrees). This is the same
...
static inline uint8_t omi_OMINO_azimuth_byte(uint8_t OMINO) {
    return (uint8_t)((OMINO & 0x01u) << 7);
}

static bool omi_verify_OMINO_azimuth_mirror(void) {
    uint8_t reference = omi_OMINO_azimuth_byte(0); /* 0x00 -> 0 degrees   */
    uint8_t antipodal = omi_OMINO_azimuth_byte(1); /* 0x80 -> 180 degrees */
...
/* ============================================================================
 * 11. THE ALGORITHMIC CLOCK, THE OMINO CENTROID, AND LOCAL/REMOTE SYNC
 * ----------------------------------------------------------------------------
 * Corrected doctrine:
 *
 ...
 *   3. OMINO defines the projective orientation of the clock.
 *   4. OMINO=0 resolves against the primary centroid: address 0x0000,
 *      azimuth 0 degrees, band (0,0,0).
 *   5. OMINO=1 resolves against the antipodal centroid: the r0-reflection
 ...
 *  12. No implementation surface may redefine the OMINO centroids or the
...
/* ---- 11.3 The OMINO centroids and the LAZY geometric vector -------------- */
...
/* There is one fixed centroid state. OMINO selects only the projective
 * orientation (0 or 180 degrees); it never redefines the centroid address. */
static inline uint16_t omi_centroid_state(uint8_t OMINO) {
    (void)OMINO;
...
static OmiGeometricVector omi_resolve_vector_to_centroid(uint16_t cons_address, uint8_t current_OMINO) {
    OmiGeometricVector vec;

    uint16_t relative = (uint16_t)(cons_address ^ omi_centroid_state(current_OMINO));
    ...
    if ((current_OMINO & 0x01u) == 1u) { /* OMINO=1: antipodal 180-degree mirror */
        raw_angle += 180.0;
...
static inline uint16_t omi_centroid_delta(uint16_t current_state, uint8_t OMINO) {
    return (uint16_t)(current_state ^ omi_centroid_state(OMINO));
}
...
static inline uint16_t omi_centroid_resolution(uint16_t current_state, uint8_t OMINO) {
    return omi_centroid_delta(current_state, OMINO);
}

/* ---- 11.5 OMINO logical orientation -------------------------------------- */

typedef struct {
    uint16_t state_word;
    uint64_t step_accumulator;
    uint8_t  current_azimuth; /* 0x00 or 0x80 -- logical orientation only */
} OmiOrientedOMINOClock;

static OmiOrientedOMINOClock omi_process_centroid_alignment(uint16_t current_state, uint8_t OMINO) {
    OmiOrientedOMINOClock clock;
    clock.state_word = current_state;
    clock.step_accumulator = 0;
    clock.current_azimuth = omi_OMINO_azimuth_byte(OMINO); /* Section 10 */
    return clock;
}
 ...
 * each side is its own centroid_resolution under the same OMINO orientation.
...
static bool omi_clock_sync_check_resolved(const OmiClockSyncPeer *local,
                                           ...
                                           uint8_t OMINO) {
    uint16_t local_sync  = omi_centroid_resolution(local->clock.state, OMINO);
    uint16_t remote_sync = omi_centroid_resolution(remote->clock.state, OMINO);
...
int main(void) {
    ...
    /* --- Section 10: LOGOS/NOMOS/PATHOS/OMINO and the Gnomonic Azimuth --- */
    ...
    printf("  FS=%u GS=%u RS=%u US=%u -> LOGOS=%u NOMOS=%u PATHOS=%u OMINO=%u\n",
           quartet.fs, quartet.gs, quartet.rs, quartet.us,
           check.logos, check.nomos, check.pathos, check.OMINO);

    OmiHammingCodeword codeword = omi_encode_hamming(quartet);
    OmiHammingStatus clean_status = omi_hamming_secded_check(&codeword, check.OMINO);
    ...
    OmiHammingStatus corrected_status = omi_hamming_secded_check(&corrupted, check.OMINO);
    ...
    printf("  The 0-degree OMINO model:\n");
    uint8_t az_ref  = omi_OMINO_azimuth_byte(check.OMINO);
    uint8_t az_flip = omi_OMINO_azimuth_byte((uint8_t)(check.OMINO ^ 1u));
    printf("    OMINO=%u -> azimuth byte 0x%02X (%.0f deg)\n",
           check.OMINO, az_ref, omi_azimuth_degrees(az_ref));
    printf("    OMINO=%u -> azimuth byte 0x%02X (%.0f deg)\n",
           (uint8_t)(check.OMINO ^ 1u), az_flip, omi_azimuth_degrees(az_flip));
    printf("    Antipodal pair is the 0x80 mirror of the reference: %s\n\n",
           omi_verify_OMINO_azimuth_mirror() ? "PASS" : "FAIL");

    /* --- Section 11: algorithmic clock, OMINO centroid, local/remote sync --- */
    printf("[11] Algorithmic clock and the 0x00 & 0-degree OMINO Centroid:\n");
    ...
    printf("  Centroid alignment check (0x0000, OMINO=0): %s\n",
           omi_verify_centroid_alignment() ? "PASS" : "FAIL");

    uint16_t diagnostic_addrs[3] = {0x0007u, 0x3030u, 0x8008u};
    uint8_t  diagnostic_OMINO[3] = {0u, 0u, 1u};
    for (int i = 0; i < 3; i++) {
        OmiGeometricVector v = omi_resolve_vector_to_centroid(diagnostic_addrs[i], diagnostic_OMINO[i]);
        printf("    addr 0x%04X (OMINO=%u) -> radius=%.2f  azimuth=%.1f deg\n",
               v.relative_address, diagnostic_OMINO[i], v.radial_distance, v.azimuth_angle);
    }

    printf("  Fixed centroid state: OMINO=0 -> 0x%04X, OMINO=1 -> 0x%04X\n",
           omi_centroid_state(0u), omi_centroid_state(1u));
    printf("  centroid_resolution(0x1234, OMINO=0) = 0x%04X\n",
           omi_centroid_resolution(0x1234u, 0u));
    printf("  centroid_resolution(0x1234, OMINO=1) = 0x%04X\n",
           omi_centroid_resolution(0x1234u, 1u));

    OmiOrientedOMINOClock rc0 = omi_process_centroid_alignment(0x0000u, 0u);
    OmiOrientedOMINOClock rc1 = omi_process_centroid_alignment(0x0000u, 1u);
    printf("  OMINO=0 -> logical azimuth=0x%02X (0 degrees)\n", rc0.current_azimuth);
    printf("  OMINO=1 -> logical azimuth=0x%02X (180 degrees)\n\n", rc1.current_azimuth);
    ...
    printf("      [exact centroid resolution, OMINO=0] %s (local=0x%04X remote=0x%04X)\n",
    ...
    printf("      [exact centroid resolution, OMINO=0] %s (local=0x%04X drifted=0x%04X)\n\n",
