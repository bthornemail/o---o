...
flowchart LR
    subgraph Forward["Forward (Low → High)"]
        A1["<b>omi</b><br/>0x03BF<br/>ο small omicron"] --> B1["<b>---</b><br/>Data Bus<br/>4-char hex blocks"]
        B1 --> C1["<b>imo</b><br/>0x039F<br/>Ο capital omicron"]
    end

    subgraph Inverted["Inverted (High → Low)"]
        C2["<b>imo</b><br/>0x039F"] --> B2["<b>---</b><br/>Data Bus<br/>(read right-to-left)"]
...
The entire Omi Object Model turns on a single linguistic palindrome: `omi---imo`. Read it forward, it says "omi"; backward, it says "imo." The three hyphens between them are not decoration — they are the Bus.
...
Everything between the omicron anchors is hyphen-delimited 4-character hexadecimal blocks (16 bits each). The hyphens act as bus dividers — clock edges that separate addressing domains. No bit can bleed across a hyphen.
