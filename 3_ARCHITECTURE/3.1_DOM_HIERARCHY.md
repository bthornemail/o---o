...
`unicode-bidi` is the activation function for chirality. Forward nodes use `id^="omi-"` with `direction: ltr`; inverted nodes use `id^="imo-"` with `direction: rtl`. The same codepoint frame can therefore be read in high-definition forward state or low-definition inverse recovery state.
...
The non-printing ASCII separators remain the canonical directional gates:
